...
public class tech extends JFrame implements ActionListener
{
	...
	public tech()
	{
		...
		new2.setBackground(new Color(128,0,0));
        	tsave = new JButton("Save", new ImageIcon("image/other/Save.gif")); 
		tsave.setForeground(Color.white); 
		tsave.setBackground(new Color(128,0,0));
		...
		tsave.setToolTipText("Save the record");	
		...
		tsave.setBounds(560,450,100,30);
		...
		gen.setBounds(190,390,150,20);
			ljo1 = new Choice();
		ljo1.add("Lecturer");
		ljo1.add("Ass.Profeser");
		ljo1.add("Profesor");
		ljo1.add("Associate Pro.");
		cex1 = new Choice();
		cex1.add("Industrial");
		cex1.add("Teching");
		cex1.add("Other");
	...
	{	
		...
		try
		{
			...
			else
			{
					...
					{
						t=0;
						PreparedStatement ps  = c.prepareStatement("insert into tech values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
