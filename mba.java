...
public class mba extends JFrame implements ActionListener
{
	...
	public mba()
	{
		...
		new1.setBackground(Color.gray);
        	bsave = new JButton("Save", new ImageIcon("image/other/Save.gif")); 
		...
		bsave.setBounds(560,450,100,30);
		...
		lqul = new Choice();
		lqul.add("ICET");lqul.add("CAT");lqul.add("Managment");
		...
		try
		{
			...
			else
			{
				...
				else
				{
					...
					{
						t=0;
						PreparedStatement ps  = c.prepareStatement("insert into mba values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
